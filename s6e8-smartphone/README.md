...
| 누적 개선 (CV) | **+0.004716** |
| 순위 | **981위 / 3,532팀 (상위 27.8%)** |

> 📕 **[상위권 솔루션 분석 → docs/TOP_SOLUTIONS.md](docs/TOP_SOLUTIONS.md)**
> 1·2위가 무엇을 했고 우리가 무엇을 놓쳤는지. **핵심: 타깃 인코딩을 '정확한 값'이 아닌
> '반올림한 구간'으로 만들어서 실패했다.** 상위권은 같은 기법으로 +0.0046을 벌었다.
...
| v3 | 05 | + 3-모델 앙상블 | 16 | **0.964890** | +0.000374 | **0.96613** |

> **CV-LB 점검**: LB가 CV보다 **+0.00124 높다.** 과적합이 있었다면 반대 방향이므로
> 검증 틀은 건전하다. 테스트 예측이 5개 폴드 모델의 평균이라 구조적으로 LB가 유리한
> 비교이기도 하다. 자세한 분석은 [PROGRESS.md 7-3절](docs/PROGRESS.md)에 정리했다.
