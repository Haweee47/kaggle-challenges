...
**작업 효율이 2.4배 차이난다.** 우리는 3걸음 걸어 1번 일하고, 저쪽은 1걸음에 1번 일한다.
무엇을 심든 이 비율이 74%인 한 천장이 낮다.

측정: d20 시점 경작 타일 9개의 **창고까지 평균 거리 5.0칸** (최대 7칸).
5x5 한 구역이면 창고 인접부터 채워 평균 2~3칸이면 충분한데, 흩어져 있었다.

## v3의 세 가지 변경
1. **밀집 배치** — 새로 심을 타일을 창고에서 가까운 순으로 고른다 (기존: 좌상단부터 스캔)
2. **연속 처리** — 유닛이 서 있는 타일에 할 일이 있으면 무조건 먼저 한다
3. **CARE 우선순위 상향** — v16rc5의 2위 행동(967회)인데 우리는 사실상 안 했다

출처: boatlee/v16-rc5-high-score-8c-4s-premium-market-lead 의 테이프 분석
      (코드를 베끼지 않고, 드러난 운영 데이터만 참고했다)
"""
