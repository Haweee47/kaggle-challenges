...
v4에서 '가격이 낮으면 기다린다'를 기각하며 얻은 결론: 기다리면 손해다.
상대가 계속 물량을 풀어 가격이 회복되지 않고, 창고 100개 상한에 걸려 폐기된다.

그래서 반대로 간다 — **상대가 팔기 전에 먼저 판다.**
가격은 매도 직전 재고로 호가되므로 먼저 파는 쪽이 비싸게 받는다.

상대 판매량은 관측에서 역산한다:
    Δ재고 = (내 판매) + (상대 판매) − (마을 소비)
내 판매를 알고 마을 소비를 추정하면 나머지가 상대 판매다. (src/frontrun.py)
"""
from kaggle_environments.envs.kaggriculture.kaggriculture import CROPS, ANIMALS

try:
    import sys as _s, os as _o
    _s.path.append(_o.path.join(_o.path.dirname(_o.path.dirname(
        _o.path.abspath(__file__))), 'src'))
    import frontrun as FR
except Exception:
    FR = None
...
P = {
    ...
    # ── 판매 선점 (v5) ──
    # ⚠️ 실측 기각. vs v16rc5: 꺼짐 26,269 / +10 25,575 / +25 25,575 / +50 25,575
    #    추가 수량을 아무리 늘려도 결과가 같다 = **팔 물건 자체가 없다.**
    #    d27 창고 비교: 우리 {밀 45} vs 상대 {밀 6, 딸기 15, 우유 3}
    #    우리는 프리미엄 상품을 아예 생산하지 못한다.
    #    -> 판매 타이밍은 '이미 130k를 생산하는 에이전트들 사이의' 승부처다.
    #       26k인 우리에게 병목은 여전히 **생산**이다.
    'front_run': 0,           # 상대가 파는 품목을 먼저·더 많이 판다
    'fr_extra': 25,           # 선점 시 추가로 파는 수량
...
_LAST_SELLS = {}          # 지난 턴 내 판매 기록 (상대 판매 역산용)
...
def act(obs):
    ...
    hot = {}
    if P['front_run'] and FR is not None:
        # 지난 턴 내 판매를 기억해 두고, 이번 관측의 재고 변화에서 빼면
        # 나머지가 상대 판매량이다.
        try:
            hot = FR.observe(obs, _LAST_SELLS.get(me), player=me)
        except Exception:
            hot = {}
    ...
    if P['front_run'] and FR is not None and hot:
        try:
            reserve = {'WHEAT': (n_animals + pending_a) * 7}
            market = FR.boost(market, hot, shed, reserve, P['fr_extra'])[:10]
        except Exception:
            pass
    _LAST_SELLS[me] = {o[1]: o[2] for o in market if o[0] == 'SELL' and len(o) > 2}
